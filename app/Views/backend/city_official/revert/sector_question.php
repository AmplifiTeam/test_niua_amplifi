...
<title>NIUA | City Official</title>
...
<h1 class="d-none"><?php echo $sectorName; ?> Survey <?php echo $surveyName; ?> Questions</h1>
<p>
<a href="<?php echo base_url('admin/city-reverted-dashboard/'.$selctedSurvey); ?>"><i class="bi bi-chevron-left"></i> Back </a>
</p>
...
if(count($chk)>0){
  $redirect=base_url()."admin/sector-revert-question/".$u_surveyId."/".$allsectorDetails["Sector_ID"];
  ...
  if(in_array($allsectorDetails["Sector_ID"],$revSec)){
?>
<li class="<?php if($u_sectorId==trim($allsectorDetails['Sector_ID'])){ echo "left_menu_active_sector"; } ?>">
  <a href="<?php echo $redirect; ?>">
    ...
    <div class="a-s-p-text"> 
      <span><?php echo $allsectorDetails['Sector']; ?></span> 
    </div>
  </a>
</li>
<?php }}}} ?>
...
<div class="total-nu1 ps-2" parent_qb_id="">
...
?>
<div class="total-nu1 ps-2 DateWithTimeSection" parent_qb_id="">
<span class="questiontitle"><?php echo $i; ?>. <?php echo $allQuestiondetails["Description"]; ?><button type="button" class="btn" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip" style="pointer-events: all;" onclick="questionDetail('<?php echo trim($allQuestiondetails["QB_ID"]); ?>')"><i class="bi bi-info-circle-fill"></i></button></span>
<div class="value">
...
<div class="total-nu1 ps-2 ParentChildConditionalList" parent_qb_id="<?=$allQuestiondetails['QB_ID']?>">
<span class="questiontitle "><?php echo $i; ?>. <?php echo $allQuestiondetails["Description"]; ?><button type="button" class="btn" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip" style="pointer-events: all;" onclick="questionDetail('<?php echo trim($allQuestiondetails["QB_ID"]); ?>')"><i class="bi bi-info-circle-fill"></i></button></span>
...
?>
<div class="show_childQuestion revertCycle">
...
<?php if(!empty($getChildAnswer) && $getChildAnswer['validator_2_status']==3){?>
<span qb-id="<?php echo $childDetail["QB_ID"] ?>" style="color: #121211;background-color: #f1b915a1;border: 1px solid #df991a; cursor:pointer;float: right;    margin-top: -50px" class="dem-but2a v2comment" href="javascript:void(0);">Revert with comment by Validator</span> <?php }?>
<span class="error_msg err" style="color: red;"></span>
</div>
<?php $child_i++;}} ?>
</div>
...
<?php if(!empty($getChildAnswer) && $getChildAnswer['validator_2_status']==3){?>
<span qb-id="<?php echo $childDetail["QB_ID"] ?>" style="color: #121211;background-color: #f1b915a1;border: 1px solid #df991a; cursor:pointer;float: right;margin-top: -50px" class="dem-but2a v2comment" href="javascript:void(0);">Revert with comment by Validator</span>
<?php }?>
...
<span qb-id="<?php echo $allQuestiondetails["QB_ID"] ?>" style="color: #121211;background-color: #f1b915a1;border: 1px solid #df991a; cursor:pointer;" class="dem-but2a v2comment" href="javascript:void(0);">Revert with comment by Validator</span>
<?php }?>
...
<!-- Validator Remark Popup -->
<div class="modal fade model-wid1" id="validatorRemarkModalPopup" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
<div class="modal-dialog">
<div class="modal-content">
<div class="modal-header">
<h1 class="modal-title fs-5" id="exampleModalLabel">Validator comment</h1>
<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
</div>
<div class="modal-body">
<div>
...
<div class="mb-2">
<textarea style="pointer-events: none;" class="form-control" id="validator_remark" placeholder="" rows="5"></textarea>
<span class="error_msg validator_remark_err" style="color: red;"></span> 
</div>
</div>
</div>
</div>
</div>
<!-- End Validator Remark popup -->
...
function saveQuestion(surveyId,sectorId,qbId,answer){
    ...
    if(surveyId!="" && sectorId!="" && qbId!=""){
          ...
          data: {'qb_id':qbId,'ans':answer,'survey_id':surveyId,'sector_id':sectorId, 'cycle':'revert'},
          dataType: "json",
          beforeSend: function(){
            //$('.loadingSection').show();
          },
...
$(document).on('change','.parent_child_conditional_option',function(){
    ...
    var sector='<?php echo trim($sectorId) ?>';
      if(questionId!="" && survey!=""  && sector!=""){
          ...
          data: {'option':selectedOption,'qb_id':questionId,'ans':questionAnswer,'survey_id':survey,'sector_id':sector, 'cycle':'revert'},
          dataType: "json",
          success: function(response){
              //console.log(response);
              if(questionId!="" && survey!=""  && sector!=""){
...
$(document).on('blur, change','.childTextBoxInput',function(){
   ...
   if(survey!="" && sector!="" && parent_questionId!="" && questionId!=""){
          ...
          data: {'option':selectedOption,'parent_qb_id':parent_questionId, 'qb_id':questionId,'ans':questionAnswer,'survey_id':survey,'sector_id':sector, 'cycle':'revert'},
          dataType: "json",
          success: function(response){
            //console.log(response);            
            if(response.status==1){
...
<script>
  $(document).on("click",".saveQuestionRemarkBtn", function(){
     ...
     $.ajax({
          ...
          data: {'qb_id':questionId,'rem':remark,'survey_id':survey,'sector_id':sector,'cycle':'revert'},
          dataType: "json",
          beforeSend: function(){
            //$('.loadingSection').show();
          },
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
            ...
            data: {'id':document_id,'survey_id':survey,'cycle':'revert'},
            dataType: "json",
            beforeSend: function(){
...
<script>
  $(document).on('click','.v2comment',function(){
    var questionId=$(this).attr('qb-id').trim();
    var survey='<?php echo trim($surveyId) ?>';
    var sector='<?php echo trim($sectorId) ?>';
    var parent_qb_id=$(this).parents('.total-nu1').attr('parent_qb_id');
    if(parent_qb_id){
      parent_qb_id = parent_qb_id.trim();
    }
    //alert(`Q :: ${questionId} Survey :: ${survey} Sector : ${sector}`);
    //Ajax call...
    if(questionId!="" && survey!="" && sector!=""){
          $.ajax({
          type:'POST',
          url: '<?php echo base_url(); ?>admin/get-validator2-comment',
          data: {'question':questionId,'survey_id':survey,'sector_id':sector,'parent_qb_id':parent_qb_id},
          dataType: "json",
          beforeSend: function(){
          //$('.loadingSection').show();
          },
          ...
          success: function(response){
              //console.log(response);
              $("#validator_remark").val(response.comment); 
              $('#validatorRemarkModalPopup').modal('toggle');         
          }
          });
    }        
  });  
...
<script>
  ...
  if(input1!="" && input2!=""){
      ...
      data: {'qb_id':parent_questionId,'ans':questionAnswer,'survey_id':survey,'sector_id':sector, 'cycle':'revert', 'input1':input1, 'input2':input2, 'operation':operation},
      dataType: "json",
      success: function(response){
          //console.log(response);
          if(response.status==1){

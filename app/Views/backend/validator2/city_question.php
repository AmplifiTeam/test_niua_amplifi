    ...
    <div class="card-body2 pb-0">
      ...
      <ul class="all-sectr-prt1">
        <?php
          ...
          $Jobdata = ($Jobdata && !empty($Jobdata))?$Jobdata:[];
          ...
          foreach($allsector as $allsectorDetails){
            ...
            $validated=getValidatedQuestionsInSector($url_survey,$getsec,$city,2);
            $redirect=base_url()."admin/city-questions2/".$url_cityUserId."/".$getsec."/".$url_survey;
        ?>
    ...
    <div class="selet-del-1">
        <div class="form-check check-1">
          ...
          <div class="rejt-1 allRejectAndApprove">
            <button id="approveAll" type="button" class="btn btn-primary new-but1 remark"><i class="bi bi-check-lg"></i> Approve </button>
            ...
            <!-- <button id="rejectAll" type="button" class="btn btn-primary new-but1 fileupload"> <i class="bi bi-x"></i> Reject </button> -->
...
<div class="value" style="width:100%; pointer-events: none;" >
...
<div class="total-nu4 ps-3" style="pointer-events:none;">
<span class="questiontitle"><?php echo $i; ?>. <?php echo $allQuestiondetails["Description"]; ?><button type="button" class="btn" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip"  onclick="questionDetail('<?php echo trim($allQuestiondetails["QB_ID"]); ?>')"><i class="bi bi-info-circle-fill"></i></button></span>
<div class="checkbox">
<?php
$options=getQuestionAllOptions($allQuestiondetails["QB_ID"]);
...
<span class="nav-item dropdown pe-3">
  <a class="nav-link nav-profile d-flex align-items-center pe-0" href="#" data-bs-toggle="dropdown" aria-expanded="false">
<button type="button" class=" nav-item dropdown btn btn-primary new-but1 rejectButtonClassToBeUpdated <?=(!empty($Jobdata) && ($Jobdata['v2_status']==1 || $Jobdata['v2_status']==2))?'NotToBeReperiledAgain':''?> <?php if($getChildAnswer["validator_2_status"]==2){ echo "bg-danger";} ?>" qb-id="<?php echo $getChildAnswer["QB_ID"] ?>"><i class="bi bi-x"></i>Reject</button>
</a>
...
<?php if(trim($allQuestiondetails['UOM_ID'])!=42 && trim($allQuestiondetails['UOM_ID'])!=43){ ?>
...
<button type="button" class="btn btn-primary new-but1 approveQuestion <?=(!empty($Jobdata) && ($Jobdata['v2_status']==1 || $Jobdata['v2_status']==2))?'NotToBeReperiledAgain':''?> <?php if($allQuestiondetails["validator_action"]==1){ echo "bg-success";} ?>" qb-id="<?php echo $allQuestiondetails["QB_ID"] ?>"><i class="bi bi-check-lg"></i>Approve</button>

<span class="nav-item dropdown pe-3">
  <a class="nav-link nav-profile d-flex align-items-center pe-0" href="#" data-bs-toggle="dropdown" aria-expanded="false">
<button type="button" class="btn btn-primary new-but1 rejectButtonClassToBeUpdated <?=(!empty($Jobdata) && ($Jobdata['v2_status']==1 || $Jobdata['v2_status']==2))?'NotToBeReperiledAgain':''?> <?php if($allQuestiondetails["validator_action"]==2){ echo "bg-danger";} ?>" qb-id="<?php echo $allQuestiondetails["QB_ID"] ?>"><i class="bi bi-x"></i>Reject</button>
</a>
<ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow profile" style="">
<li class="">
...
<button type="button" class="btn btn-primary new-but1 openRevertWithCommentPopup <?=(!empty($Jobdata) && ($Jobdata['v2_status']==1 || $Jobdata['v2_status']==2))?'NotToBeReperiledAgain':''?> <?php if($allQuestiondetails["validator_action"]==3){ echo "bg-warning";} ?>" qb-id="<?php echo $allQuestiondetails["QB_ID"] ?>"> 
<i class="bi bi-chat-right-text-fill"></i>&nbsp;Revert with Comment</button>
...
<div class="v1commentSection"></div>
...
<!-- <div class="modal-footer">
<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
<button type="button" class="btn btn-primary saveQuestionRemarkBtn" question="935">Submit</button>
</div> -->
</div>
</div>
</div>
...
<script>  
  $(document).on("click",".openRevertWithCommentPopup", function(){
    ...
    $.ajax({
      ...
      url: '<?php echo base_url(); ?>admin/get-validator1-comment',
      ...
      success: function(response){
        ...
        $(".v1commentSection").html('');
        $(".v1commentSection").html(response.v1_comment);
        var v1Comment=response.comment.trim();
        $("#revertWithComment").val(v1Comment);
  ...
  $(document).on("click",".approveQuestion", function(){
    if($(this).hasClass('NotToBeReperiledAgain')){
      ...
      if (result.value) {
        $.ajax({
            type:'POST',
            url: '<?php echo base_url(); ?>admin/validator-approve2',
            ...
            success: function(response){
              //console.log(response);validator-revert-with-comment2
...
<script>  
  $(document).on("click","#approveAll", function(){
    ...
    //       if(!$(che).hasClass('NotToBeReperiledAgain')){
    ...
    if(totalChecked==0){
      ...
      if (result.value) {
        $.ajax({
            type:'POST',
            url: '<?php echo base_url(); ?>admin/validator-approve-all2',
            ...
            success: function(response){
              ...
              if(response.status==1){
                ...
                //       $('#updateV2Status_'+survey+'_'+sector+'_'+qbid).find('.approveQuestion').addClass('bg-success');
                //       $('#updateV2Status_'+survey+'_'+sector+'_'+qbid).find('.rejectQuestion').removeClass('bg-danger');
                //       $('#updateV2Status_'+survey+'_'+sector+'_'+qbid).find('.openRevertWithCommentPopup').removeClass('bg-warning');
                //   })
                ...
                var revertWithCmt=$(".revertWithCommentCnt").text();
                var total=parseInt(total_approved)+parseInt(total_rejected)+parseInt(revertWithCmt);
                //$(".validatedCnt").text(total);
                // $(".validatedCnt").text(response.total_approved);
  ...
  $(document).on("click",".rejectQuestion", function(){
    ...
    if($(this).parents('.childQuestionDivOptions').length){
      ...
      if (result.value) {
        $.ajax({
            type:'POST',
            url: '<?php echo base_url(); ?>admin/validator-reject2',
  ...
  $(document).on("click","#rejectAll", function(){
    ...
    if(totalChecked==0){
      ...
      if (result.value) {
        $.ajax({
            type:'POST',
            url: '<?php echo base_url(); ?>admin/validator-reject-all2',
            ...
            success: function(response){
              ...
              if(response.status==1){
                ...
                // $.each(selectQuestinList,function(i,qbid){
                //     $('#updateV2Status_'+survey+'_'+sector+'_'+qbid).find('.approveQuestion').removeClass('bg-success');
                //     $('#updateV2Status_'+survey+'_'+sector+'_'+qbid).find('.rejectQuestion').addClass('bg-danger');
                //     $('#updateV2Status_'+survey+'_'+sector+'_'+qbid).find('.openRevertWithCommentPopup').removeClass('bg-warning');
                // })
                var questionSection = $('.remove-2');
                $.each(selectQuestinList,function(i,qbid){
                ...
                var total=parseInt(total_approved)+parseInt(total_rejected)+parseInt(revertWithCmt);
                //$(".validatedCnt").text(total);
                // $(".validatedCnt").text(response.total_rejected);
...
<script>
  ...
  $(document).on('click','.addToBookmark',function(){
    ...
    if($(this).hasClass('bi-star-fill clr2')){ // Checked
       ...
       $.ajax({
          type:'POST',
          url: '<?php echo base_url(); ?>admin/validator-bookmark-question2',
...
<script>
  $('#filter_validator_question').on('change', function(){
    ...
    $.ajax({
          type:'POST',
          url: '<?php echo base_url(); ?>admin/filter-validator-question2',
...
<script>  
  /*$(document).on('click','.countFilter',function(){
    var filter_type=$(this).attr('data-val').trim();
    var survey='<?php echo $url_survey ?>';
    var sector='<?php echo $url_sectorId ?>';
    var cityUserId='<?php echo $url_cityUserId ?>';
    alert(`Survey=>${survey}, Sector=>${sector}, City=>${cityUserId} Filter Type :: ${filter_type}`);
  });*/
  ...
  $(document).on('click','#saveRevertWithComment',function(){
    ...
    if(questionId!=""){
        ...
        $.ajax({
          type:'POST',
          url: '<?php echo base_url(); ?>admin/validator-revert-with-comment2',
          ...
          success: function(response){
            ...
            if(response.status==1){
                  ...
                  // $('#updateV2Status_'+survey+'_'+sector+'_'+questionId).find('.rejectQuestion').removeClass('bg-danger')
                  // $('#updateV2Status_'+survey+'_'+sector+'_'+questionId).find('.openRevertWithCommentPopup').addClass('bg-warning')

            if(parent_qb_id > 0){
                  var checkListElements = $('body').find('.heckForChangeInStatusForRevert_'+parent_qb_id);

...
<div class="row">
  ...
  <div class="col-md-4 mt-2">
    <?php $selectedSurvey = (session('V2_selected_Survey') && session('V2_selected_Survey')!='')?session('V2_selected_Survey'):''; ?>
...
<script>
  $(document).on('click','.addToPriority',function(){
    ...
    if($(this).hasClass('bi-star-fill clr2')){ // Checked
       ...
       $.ajax({
          type:'POST',
          url: '<?php echo base_url(); ?>admin/add-priority-city2',
...
<script>
  $('#Survey_ID').change(function(){
    ...
    $.ajax({
              type:'POST',
              url: '<?php echo base_url(); ?>admin/get-kpi-list2',
...
<script>
  function action_by_v2(jobId,action){
    if(jobId!="" && action!=""){
      Swal.fire({
      icon: "question",     
      ...
      $.ajax({
        type:'POST',
        url: '<?php echo base_url(); ?>admin/validator-final-action',
        data: {'job':jobId,'action':action},
        dataType: "json",
        beforeSend: function(){
          //$('.se-pre-con').show();
        },
        ...
        success: function(response){
          ...
          if(response.status==1){
              ...
              Swal.fire({icon: 'error',text: response.msg}).then((result) => {
              window.location.reload();                        
              });
          }
        }
